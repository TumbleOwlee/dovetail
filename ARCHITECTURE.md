...
A key event is read by a blocking thread in `event`, sent over a channel, and handed to `app`, which offers it to the open dialog, then the open command line, then the main view. A submitted command line goes through `command::parse` back into `app`. Writes go from `app` through `config::store` to disk; a confirmed dialog derives profiles through `config::profile` first. Opening the dialog on a configured repository queues a `FetchRequest`; the loop spawns it with a shared `reqwest::Client` and the outcome returns as an `event::Message` that `app` hands to the dialog. Every frame is rendered from `app` state alone.
