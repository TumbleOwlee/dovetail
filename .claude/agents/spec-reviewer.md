...
**Standards** — `AGENTS.md` `## Conventions — code` (typed errors, typed domain values, no crash on external input, file-splitting rule, dependency policy, states-as-variants rule); test naming and ID citation placement; unflagged semver-relevant public surface changes; comment hygiene.
