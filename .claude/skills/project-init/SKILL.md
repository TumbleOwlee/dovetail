...
- **Heading text survives verbatim.** `spec-author`/`spec-planner`/`spec-implementer`/`spec-reviewer` never read `AGENTS.md` whole: each batch-pulls the headings its own file names (`## Spec-driven`, `## TDD — fixed order, every stage`, `## Where to look for task X`, `## Build / test / lint`, `## Conventions — reading`/`— code`/`— text`, `## Scope boundaries — ask before`) with `extract-section.sh`. A reworded heading silently empties that agent's read. No separate excerpt file exists — `AGENTS.md` is the single copy.
