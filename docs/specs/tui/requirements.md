...
**TU-R-065** — Enter on the Git Remote tab with a selected row shall request that pull request's details and open the details overlay, centered and covering most of the screen, reading `Loading pull request..` until the details arrive, or the error message when the request failed.
