...
**GH-R-024** — The application shall post a conversation comment on a pull request or an issue through the GraphQL mutation `addComment(input: {subjectId: ID!, body: String!}) { commentEdge { node { id } } }` at the v4 endpoint with the section's token, the subject id the node id GH-R-011 or GH-R-015 delivered; a transport failure, a non-success status, a GraphQL error or a missing node yields the matching typed error.
**GH-R-025** — The issue details query (GH-R-010) and the pull request details query (GH-R-015) shall also request the subject's `viewerCanUpdate`, carried as whether the viewer may update its body.
**GH-R-026** — The application shall update a conversation comment of an issue or a pull request through the GraphQL mutation `updateIssueComment(input: {id: ID!, body: String!}) { issueComment { id } }` at the v4 endpoint with the section's token, the id the comment's timeline item carries (GH-R-017); a transport failure, a non-success status, a GraphQL error or a missing node yields the matching typed error.
**GH-R-027** — The application shall update an issue's body through the GraphQL mutation `updateIssue(input: {id: ID!, body: String!}) { issue { id } }` at the v4 endpoint with the section's token, the id the issue details query delivered; a transport failure, a non-success status, a GraphQL error or a missing node yields the matching typed error.
**GH-R-028** — The application shall update a pull request's body through the GraphQL mutation `updatePullRequest(input: {pullRequestId: ID!, body: String!}) { pullRequest { id } }` at the v4 endpoint with the section's token, the node id GH-R-015 delivered; a transport failure, a non-success status, a GraphQL error or a missing node yields the matching typed error.
