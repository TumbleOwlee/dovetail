...
**GH-R-012** — The application shall load a repository's pull requests through the GraphQL API by querying `repository(owner, name)`'s `pullRequests` ordered by `UPDATED_AT` descending, 100 per page, requesting the next page with the previous page's `endCursor` while `hasNextPage` is true, each carrying number, title, state (`OPEN`, `MERGED` or `CLOSED`), draft flag, author login, head and base branch names and `updatedAt`.
